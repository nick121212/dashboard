...
app.use(logger('dev'))
    ...
    .use(query());
app.use(serveStatic("public", config.static_path));
...
consign({})
    ...
    .into(app, db, commonError, config);
...
app.use((err, req, res, next) => {
    let httpStatusErr = commonError.HttpStatusError(err);
    ...
    if (err.status === 404) {
        ...
        errorInfo = `*** ${err.status}: ${originalUrl}\nStack: ${httpStatusErr.stack}\n${clientInfo}\n***`;
    }
    // Log
    console.error(errorInfo);
    res.status(err.status || 400);
    res.json({error: httpStatusErr});
});
