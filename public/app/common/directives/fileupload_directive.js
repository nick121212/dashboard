...
define(["require", "exports", "modules/page/controllers/manager_con"], function (require, exports, manager_con_1) {
    ...
    var FileUploadController = (function (_super) {
        ...
        FileUploadController.prototype.doShowList = function () {
        ...
        FileUploadController.prototype.initUploader = function () {
            var _this = this;
            this.initServerOnlyInterfaces(["servers-fetch"], {
                ...
                _this.$q.all(_this.dealInterfaces(results.interfaceObjects, results.serverInterfaces, results.item, results.params)).then(function (results) {
                    var result = results["servers-fetch"];
                    if (result.rows && result.rows.length) {
                        var server = result.rows[0];
                        var uploader = _this.uploader = new _this.FileUploader({
                            url: server.verb.toLowerCase() + "://" + server.baseUrl + ":" + (server.port || 80) + _this.uploadPath,
                            alias: "upfile",
                            autoUpload: true
                        });
                        ...
                        console.warn("\u627E\u4E0D\u5230\u9ED8\u8BA4\u7684\u670D\u52A1\u5668\uFF0Ctype\u4E3A@{this.type}");
                    }
                });
            });
        };
        ...
        FileUploadDirective.directive = [
            ...
            function ($compile, FileUploader) {
                var directive = {
                    ...
                    restrict: "EA",
                    transclude: true,
                    templateUrl: "app/common/directives/fileupload.tpl.html",
                    scope: true,
                    bindToController: {
                        uploadPath: "=",
                        uploadType: "=",
