    ...
    initUploader() {
        ...
        this.initServerOnlyInterfaces(["servers-fetch"], {
            ...
            this.$q.all(this.dealInterfaces(results.interfaceObjects, results.serverInterfaces, results.item, results.params)).then((results)=> {
                let result = results["servers-fetch"];

                if (result.rows && result.rows.length) {
                    let server = result.rows[0];

                    let uploader = this.uploader = new this.FileUploader({
                        url: server.verb.toLowerCase() + "://" + server.baseUrl + ":" + (server.port || 80) + this.uploadPath,
                        alias: "upfile",
                        autoUpload: true
                    });
        ...
        //
        //
        // this.Restangular.all("servers").doGET.bind(this, null, {
        //     filter: {
        //         where: {
        //             type: this.uploadType,
        //             isDefault: true
