    ...
    confirm(opts:IConfirm, serverInterfaces, filterData:Function, action:any) {
        ...
        if (this.selected.length > 0 || opts.ignoreSelection) {
            this.$mdDialog.show(confirm).then(() => {
                this.deferred = this.$q.all(this.dealInterfaces(action.interfaceObjects, serverInterfaces, (filterData.call(this, this.selected)) || this.selected, this.queryData)).then((data) => {
