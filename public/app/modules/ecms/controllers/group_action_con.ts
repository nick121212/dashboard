...
export class GroupActionManagerController extends PageManagerFormController {
     ...
     */
    initInterfaces() {
        let groupActions = _.map(this.groupActions, (ga) => {
            return ga;
        });
        let interfaces = [];
        let actions = _.keyBy(this.actions, "key");

        _.each(groupActions, (d)=> {
            let action = actions[d["actionKey"]];

            if (action) {
                try {
                    let inters = JSON.parse(action.interfaces);
