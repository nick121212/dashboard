...
exports = module.exports = (app, db, errors) => {
    ...
    let executeCommand = (server, modelInstance, filter, data, access_token, site) => {
        let params = {
        ...
        try {
            modelInstance.params = JSON.parse(modelInstance.params)
        } catch (e) {
        }
...
//                                 console.log(utils.dataConvert(con, d[con.key]));
        ...
        return new Sequelize.Promise((resolve, reject) => {
            ...
            if (!modelInstance.baseUrl) {
                uri = server.verb.toLowerCase() + "://" + server.baseUrl + (server.port ? (":" + server.port) : '') + modelInstance.api;
            } else if (server && server.baseUrl) {
                uri = server.verb.toLowerCase() + "://" + modelInstance.baseUrl + ":" + (modelInstance.port ? (":" + modelInstance.port) : '') + modelInstance.api;
            } else {
...
//                     console.log(err);
                    ...
                    if (result.state !== "SUCCESS") {
                        return reject(new errors.HttpStatusError(400, result.state || "错误"));
                    }
//                     console.log("------------------resolve-----------------------------");
                    resolve(result);
                });
    ...
    return (interfaceKey, filter, data, access_token, site) => {
        ...
        return new Sequelize.Promise((resolve, reject) => {
            ...
            async.auto({
                ...
                execute: ["server", (cb, results) => {
                    executeCommand(results.server, results.inteface, filter, data, access_token, site).then((result) => cb(null, result), cb);
                }]
            }, (err, results) => {
                if (err) {
                    return reject(err);
